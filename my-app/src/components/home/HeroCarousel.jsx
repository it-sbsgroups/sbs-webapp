...
export default function HeroCarousel() {
  ...
  const [isHovered, setIsHovered] = useState(false);        // Pause autoplay on hover
  const [slides, setSlides] = useState([]);                // Slides from backend
  const [settings, setSettings] = useState(CAROUSEL_DEFAULTS); // Carousel settings

  const videoRef = useRef(null); // Reference to the currently playing video element
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // On error, keep default settings (no action needed)
      }
    })();
  }, []);
  ...
  // ----- HELPER FUNCTIONS -----
  // Convert string/boolean values to a proper boolean
  const toBool = (v) => v === true || v === "true";

  // Determine media type and video behaviour for the current slide
  ...
  // When a non‑looping video ends, move to the next slide
  const handleVideoEnded = () => {
    if (isVideo && !isLoopingVideo) nextSlide();
  };

  // ----- RELOAD & PLAY VIDEO ON SLIDE CHANGE -----
  useEffect(() => {
    if (videoRef.current) {
  ...
  // ----- AUTOPLAY TIMER (only for non‑video slides) -----
  useEffect(() => {
    // Do not start timer if:
    // - hovered
    // - video is looping (it controls itself)
    // - current slide is a video (video events handle transition)
    // - autoplay is disabled globally
  ...
  // Returns alignment classes based on layoutType (LEFT, CENTER, RIGHT)
  ...
  // Returns justify classes for the CTA button group
  ...
  // Builds the background style for non‑video slides (solid, gradient, image)
  ...
  // Resolves a button style's background (solid or gradient)
  ...
  // ----- TRANSITION STYLES (slide, fade, flip, book, cube, drop) -----
  ...
  // Empty state – show a placeholder when no slides exist
  if (!slides.length) {
    return (
  ...
  return (
    /**
     * MAIN CONTAINER
     * - Width: always 100% of viewport (w-screen) + no extra margins (mx-0)
     * - Height: controlled by backend via settings.carouselHeight
     * - Overflow hidden to prevent scrollbars
     */
    <div
      className="relative w-screen max-w-full overflow-hidden bg-gray-900 mx-0"
      style={{ height: settings.carouselHeight }}
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* 3D perspective wrapper – needed for cube, flip, book transitions */}
      <div
          ...
          return (
            <div
              ...
              style={{
                ...
                // For flip/book/cube, we need backface hidden to avoid showing the back side
                backfaceVisibility:
            ...
            >
              ...
              {/* ----- CONTENT (badge, title, description, CTAs) ----- */}
              <div
              ...
              >
                ...
                >
                  ...
                  {slide.title && (
                    <h1
                      ...
                      style={{
                        ...
                        fontSize:
                          slide.titleStyle?.fontSize === "text-6xl"
                            ? "3.75rem"
                            : slide.titleStyle?.fontSize === "text-5xl"
                            ? "3rem"
                            : slide.titleStyle?.fontSize === "text-4xl"
                            ? "2.25rem"
                            : slide.titleStyle?.fontSize === "text-3xl"
                            ? "1.875rem"
                            : slide.titleStyle?.fontSize === "text-2xl"
                            ? "1.5rem"
                            : "3rem",
                        lineHeight:
                          slide.titleStyle?.lineHeight === "leading-tight"
                            ? "1.25"
                            : slide.titleStyle?.lineHeight === "leading-snug"
                            ? "1.375"
                            : slide.titleStyle?.lineHeight === "leading-normal"
                            ? "1.5"
                            : "1.25",
                  ...
                  {slide.description && (
                    <p
                      ...
                      style={{
                        ...
                        fontSize:
                          slide.descriptionStyle?.fontSize === "text-xl"
                            ? "1.25rem"
                            : slide.descriptionStyle?.fontSize === "text-lg"
                            ? "1.125rem"
                            : slide.descriptionStyle?.fontSize === "text-base"
                            ? "1rem"
                            : slide.descriptionStyle?.fontSize === "text-sm"
                            ? "0.875rem"
                            : "1.125rem",
                        lineHeight:
                          slide.descriptionStyle?.lineHeight === "leading-relaxed"
                            ? "1.625"
                            : slide.descriptionStyle?.lineHeight === "leading-normal"
                            ? "1.5"
                            : "1.625",
                  ...
                  {/* CTAs (support multiple buttons via ctas array, or fallback to single ctaText) */}
                  {(() => {
                      ...
                      ? [
                          {
                            text: slide.ctaText,
                            link: slide.ctaLink,
                            openInNewTab: slide.ctaOpenInNewTab,
                            style: slide.ctaButtonStyle,
                          },
                        ]
                      : [];
                    ...
                    return (
                      <div
                        className={`pt-2 flex flex-wrap gap-3 ${getButtonJustifyClass(
                          slide.layoutType
                        )}`}
                      >
                        {ctas.map((cta, i) =>
