// components/public/Header.jsx (DYNAMIC VERSION)
"use client";
...
export default function Header() {
  ...
  const [openCategoryId, setOpenCategoryId] = useState(null); // which category's subcategory flyout is open
  ...
  // ===== DYNAMIC DATA (Replace with API fetch) =====
  const [headerData, setHeaderData] = useState(headerDummyData);

  // ===== PRODUCTS DROPDOWN: live categories + subcategories from backend =====
  ...
  // Fetch header config from the backend; merge over bundled defaults so any
  // section that hasn't been saved yet still renders.
  useEffect(() => {
    let active = true;
    (async () => {
      try {
  ...
  useEffect(() => {
    ...
    setResults(searchAll(q, { limit: 8 }));
    setShowResults(true);
    setActiveIdx(-1);
  }, [searchQuery]);
  ...
  const handleSearchSubmit = (e) => {
    ...
    else if (searchQuery.trim()) goTo(`/products?search=${encodeURIComponent(searchQuery.trim())}`);
  };
  ...
  // handleLogout removed — no login/account UI on the public header anymore

  // ===== DYNAMIC DATA EXTRACTION =====
  const { branding, primaryNavigation } = headerData;
  ...
  // Mobile links: primary nav + every category (each opening the filtered product list)
      ...
      <div className="absolute left-0 right-0 top-full mt-2 rounded-2xl border border-gray-100 bg-white shadow-2xl z-[100] overflow-hidden">
        {results.length > 0 ? (
          <>
            ...
            <button onClick={() => goTo(`/products?search=${encodeURIComponent(searchQuery.trim())}`)}
    ...
    <header className="sticky top-0 z-50 w-full border-b-white/20 border-white-90 bg-white/70 shadow-sm backdrop-blur-md">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8 gap-4">
        ...
        <div ref={searchBoxRef} className="hidden md:flex items-center flex-1 max-w-xs relative">
          <form onSubmit={handleSearchSubmit} className="w-full relative">
            <input 
              ...
              placeholder="Search products, news, distributors..."
        ...
        <nav className="hidden lg:flex items-center gap-1 xl:gap-2">
          ...
          {/* ===== PRODUCTS MEGA-DROPDOWN — categories/subcategories from backend ===== */}
          {categories.length > 0 && (
            <div className="relative" onMouseEnter={() => setIsDropdownOpen(true)} onMouseLeave={() => { setIsDropdownOpen(false); setOpenCategoryId(null); }}>
              <button
              ...
              >
                <span>Products</span>
                <svg className={`h-3 w-3 transition-transform duration-200 ${isDropdownOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24">
              ...
              {isDropdownOpen && (
                <div className="absolute left-0 top-full mt-1 flex rounded-xl border border-gray-100 bg-white shadow-2xl z-30 overflow-hidden">
                      ...
                      return (
                        <div key={cat.id} onMouseEnter={() => hasSubs && setOpenCategoryId(cat.id)}>
                          <Link
                            ...
                            onClick={() => { setIsDropdownOpen(false); setOpenCategoryId(null); }}
                  ...
                  {/* Level 2: Nested subcategory flyout — only rendered for the hovered category */}
                  {openCategoryId && (() => {
      ...
      {isMenuOpen && (
        <div className="border-t border-gray-100 bg-white px-4 py-5 shadow-xl lg:hidden absolute top-20 left-0 w-full max-h-[calc(100vh-5rem)] overflow-y-auto z-10">
          <nav className="flex flex-col space-y-1.5">
            ...
            {/* Login button removed per request — mobile menu has no auth section */}
          </nav>
        </div>
      )}
    </header>
