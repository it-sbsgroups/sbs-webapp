  ...
  return (
    <div className="bg-white rounded-lg shadow-[0_18px_45px_rgba(0,0,0,0.12)] p-10 lg:p-12 mb-10">
      <div className="flex flex-col lg:flex-row lg:items-center gap-10">
        ...
        <div className="flex-1 flex flex-col justify-center space-y-6 text-[17px] text-gray-700 leading-8">{children}</div>
      </div>
    </div>
  );
};
...
export default function PublicAboutPage() {
  ...
  // Live counts — computed from real data, not hardcoded.
  const yearsOfExperience = new Date().getFullYear() - 2005;
  const [stats, setStats] = useState({ brands: 0, products: 0, clients: 0 });
  const [statsLoading, setStatsLoading] = useState(true);

  useEffect(() => {
    Promise.all([
      // Active, publicly-listed brands = how many brand distributorships we currently hold.
      apiClient.get("/brands/public/list").then((r) => (Array.isArray(r) ? r : r?.data ?? [])).catch(() => []),
      // pageSize:1 — we only need the `meta.total` count, not the actual product rows.
  ...
  return (
    <div className="bg-white min-h-screen font-sans text-slate-800 antialiased">
      ...
      <div id="mission" className="relative text-white overflow-hidden">
        <div className="absolute inset-0 opacity-[1] brightness-90" style={{ backgroundImage: "url('https://res.cloudinary.com/dhrnoojwo/image/upload/v1786091523/ChatGPT_Image_Aug_6_2026_06_37_17_PM_ljm7ua.png')", }} />
        <div className="max-w-6xl mx-auto px-4 py-20 md:py-28 text-center space-y-5 relative">
          <span className="text-[15px] font-black text-white bg-[#557b00] border border-lime-400/30 px-3 py-1 rounded-full uppercase tracking-widest">About {companyName}</span>
          <p></p>
          <h1 className="text-3xl md:text-5xl font-black tracking-tight max-w-3xl mx-auto">{tagline}</h1>
          <p className="text-sm md:text-base text-white font-medium max-w-xl mx-auto">Every product we supply, every partnership we build, is held to one standard — reliability our clients can plan their operations around.</p>
        </div>
      </div>
      <div className="max-w-6xl mx-auto px-4 pt-16 md:pt-20 space-y-20">
      ...
      {/* ---------------- OUR JOURNEY (Full 100% screen width section) ---------------- */}
      <LazySection id="our-journey" minHeight={480} className="w-full my-16 md:my-20">
        <OurJourney />
      </LazySection>

      <div className="max-w-6xl mx-auto px-4 pb-16 md:pb-20 space-y-20">
        ...
        <LazySection id="employees">
          <h1 className="text-5xl font-bold text-black text-blue-950 text-center">Our Valuable <span className="text-lime-600">Team</span></h1>
