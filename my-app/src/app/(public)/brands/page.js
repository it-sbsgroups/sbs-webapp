...
export default function BrandsDirectoryPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await brandsApi.getPublic(false); // isOwnBrand: false
  ...
  return (
    <div className="bg-slate-50 min-h-screen font-sans text-slate-800 antialiased">
      {/* ══════════════════ BREADCRUMB BAR (same style as Own Brands) ══════════════════ */}
      <div className="bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 py-4 px-4 md:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="max-w-2xl">
              ...
              <p className="text-xs md:text-sm text-slate-500 font-medium mt-2 leading-relaxed">
                Explore the manufacturers and distribution partners we represent across our industrial supply network.
              </p>
            </div>
        ...
        {filteredBrands.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-2xl border border-slate-200">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Refine your active keyword or switch categories tab.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-4">
            {filteredBrands.map((brand) => (
              <Link
                ...
                className="group block bg-white border border-slate-200/80 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
              >
                {/* Logo */}
                ...
                {/* Footer */}
                <div className="px-3 py-2 bg-slate-50 border-t border-slate-100 text-[9px] text-slate-500 font-medium truncate">
                  <h3 className="text-black font-black text-sm md:text-base leading-tight drop-shadow-lg text-center">
                    {brand.brandName}
                  </h3>
                </div>
              </Link>
